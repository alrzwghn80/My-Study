...
Each `start`/`resume` Server Action issues a fresh random `activeToken`, stored on `StudySession.activeToken` and returned to the calling tab only. That tab holds the token in memory (React state, not localStorage — it must not leak to other tabs). Every subsequent `pause`/`resume`/`complete`/heartbeat call from that tab includes the token; the server rejects the call (without mutating anything) if it doesn't match the session's current token.

Other tabs are read-only observers: they poll session state via a `BroadcastChannel` message that the owning tab posts on every state change (start/pause/resume/complete), and re-render as "Session running in another tab — view only" if they don't hold the matching token. A tab that reloads and finds a session already RUNNING with a token it doesn't have shows the same read-only view, not a fresh Start button — this is what prevents `Tab A = RUNNING, Tab B = RUNNING` (spec §65).
